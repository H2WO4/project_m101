...
interface InitMessageData {
  ...
  stats: StatsData;
  trafficSegments?: { density: number }[];
}
...
type WebSocketMessage =
  | { type: "init"; data: InitMessageData }
  | { type: "update"; data: UpdateMessageData }
...
interface LeafletMap {
  ...
  removeLayer(layer: LeafletPolyline): void;
}
...
interface D3ScaleBand {
  (value: string): number;
  domain(domain: string[]): D3ScaleBand;
  range(range: [number, number]): D3ScaleBand;
  padding(value: number): D3ScaleBand;
  bandwidth(): number;
}

interface D3Line {
  x(fn: (d: TrafficPoint) => number): D3Line;
  y(fn: (d: TrafficPoint) => number): D3Line;
  curve(curve: unknown): D3Line;
}

interface D3Axis {
  ticks(count: number): D3Axis;
  tickFormat(format: (d: Date) => string): D3Axis;
}

interface D3Namespace {
  select(selector: string): D3Selection;
  ...
  axisBottom(scale: unknown): D3Axis;
  axisLeft(scale: unknown): D3Axis;
  timeFormat(format: string): (d: Date) => string;
}

declare const L: LeafletNamespace;
declare const d3: D3Namespace;
...
function handleIncomingData(data: WebSocketMessage): void {
  ...
  if (type === "init") {
    ...
    if (msgData.trafficSegments) {
      msgData.trafficSegments.forEach((segment: { density: number }) => {
...
const vehicleMarkers = new Map<number, LeafletMarker>();
...
function updateTrafficChart(newValue: number): void {
  ...
  const width: number = Number.isNaN(parsedWidth) ? 400 : parsedWidth;
  const height: number = 200;
  const margin = { top: 10, right: 20, bottom: 30, left: 50 };
...
function drawRoute(routeData: RouteData): void {
  ...
  const existingLine = routeLines.get(id);
